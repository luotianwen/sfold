$(function(){
	...
	//点击颜色
	$("label").on("click","input#color",function(){
		colors = new Array();
	...
	//点击规格
	$("label").on("click","input#spec",function(){
		specs = new Array();
	...
	//修改颜色
	$("label").on("change","input#c_value",function(){
	...
	//修改规格
	$("label").on("change","input#s_value",function(){
		var id = $(this).prev().val();
		var name = $(this).val();
...
function add_color_property(_this){
	...
	if(specs.length==0){
		...
		html += '<td class="center">'+
				'	<span>'+name+'</span>'+
				'	<input id="color" value="'+name+'" type="hidden" class="form-control" placeholder="" type="text">'+
				'</td>';
		html += add_html+'</tr>';
	}else{
