...
<c:forEach items="${orderInfos }" var="item">
	<tr class="head">
		<td colspan="8">
			<span style="margin-right: 20px;">
				订单编号： <a target="_blank" href="订单url">${item.orderId}</a>&nbsp;
			</span>
			<span style="margin-right: 20px;">
	...
	<tr class="content">
		<td>
			<c:forEach items="${item.orderInfos }" var="itemc">
				<c:forEach items="${itemc.orderDetailInfos }" var="detail">
		...
		<td>
			<c:if test="${item.porderInfodiscounts==null }">
				<span>无优惠信息</span>
			</c:if>
			<c:forEach items="${item.porderInfodiscounts }" var="itemp">
				<span>
		...
		<td>
			<c:if test="${itemc.expressName==null||itemc.expressName=='' }">
				<span>暂无信息</span>
			</c:if>
			<c:if test="${itemc.expressName!=null&&itemc.expressName!='' }">
				<span v-else>快递名:${itemc.expressName}<br>快递编号:${itemc.expressNumber}</span>
			</c:if>
		</td>
