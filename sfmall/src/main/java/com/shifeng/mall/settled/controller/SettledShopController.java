...
public class SettledShopController extends BaseController {
    ...
    public ModelAndView redirect(int next){
    	if (next == 1) {
			return new ModelAndView("redirect:/settled/check_in_1");
		} else if (next == 2) {
			return new ModelAndView("redirect:/settled/check_in_2");
		} else if (next == 3) {
			return new ModelAndView("redirect:/settled/check_in_3");
		} else if (next == 4) {
			return new ModelAndView("redirect:/settled/check_in_4");
		} else if (next == 5) {
			return new ModelAndView("redirect:/settled/check_in_5");
		}else{
    ...
    public Map<String,String> check_in_save(HttpSession session){
    	...
    	try {
    		int next = merchantssettledServiceImpl.findNext(user.getuId());
			if(next==0){
				merchantssettledServiceImpl.saveMerchantssettled(user);
	    	}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
