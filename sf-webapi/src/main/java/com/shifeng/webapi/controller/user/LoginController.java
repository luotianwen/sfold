...
public class LoginController extends BaseController{
	...
	public ReqResponse<UserDTO> login(String version,String ticket,String account,String password,String sign){
		...
		try {
			//是否能继续获取访问
			if(!this.isGoOnVisit(ticket, "login", req)){
				return req;
			}
			//验证版本号
			switch (version) {
			case ApiVersion.V_1_0_0:
				...
				if(this.checkTicket(ticket,req)){
						//验证签名
						if(Objects.equal(sign, MD5Util.hex(version+ ticket + account+ password))){
	...
	public ReqResponse<UserDTO> wxlogin(String version,String ticket,String code,String sign){
		...
		try {
			//是否能继续获取访问
			if(!this.isGoOnVisit(ticket, "wxlogin", req)){
				return req;
			}
			//验证版本号
			switch (version) {
			case ApiVersion.V_1_0_0:
				...
				if(Objects.equal(sign, MD5Util.hex(version+ ticket + code))){
					ReqResponse<SysUser> user = weiXinService.login(appid, secret, code);
					if(user.getCode() == 0){
						UserDTO loginUser = new UserDTO(user.getData());
