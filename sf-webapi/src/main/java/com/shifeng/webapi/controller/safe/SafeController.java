...
public class SafeController extends BaseController{
	...
	public ReqResponse<String> checkPhoneExists(String version,String ticket,String phone,String sign){
		...
		//是否能继续获取访问
		if(!this.isGoOnVisit(ticket, "safe/checkPhoneExists", req)){
			return req;
		}
		//验证版本号
		switch (version) {
			case ApiVersion.V_1_0_0:
				...
				if(this.verifySign(sign, version+ ticket + phone,req)){
					boolean bl = registerService.checkAccountExists(phone);
					if(bl){
						req.setCode(10006);
						req.setMsg("账户不存在");
					}else{
	...
	public ReqResponse<String> mobileCode(String version,String ticket,String phone,String sign){
		...
		//是否能继续获取访问
		if(!this.isGoOnVisit(ticket, "safe/mobileCode", req)){
			return req;
		}
		//验证版本号
		switch (version) {
	...
	public ReqResponse<String> checkMobileCode(String version,String ticket,String authcode,String sign){
		...
		//是否能继续获取访问
		if(!this.isGoOnVisit(ticket, "safe/checkMobileCode", req)){
			return req;
		}
		//验证版本号
		switch (version) {
	...
	public ReqResponse<String> findPwd(String version,String ticket,String newPassword,String sign){
		...
		//是否能继续获取访问
		if(!this.isGoOnVisit(ticket, "safe/findPwd", req)){
			return req;
		}
