										...
										<div class="table-responsive">
											<table class="table table-striped">
												...
												<tbody v-for="(orderInfo,index) in orderInfos">
													<tr class="head">
														<td colspan="8">
															<span style="margin-right: 20px;">
																订单编号： <a target="_blank" href="订单url">{{orderInfo.orderId}}</a>&nbsp;
															</span>
															<span style="margin-right: 20px;">货款金额： ￥{{orderInfo.order_total_price}}</span>
													...
													<tr class="content" v-for="orderInfoc in orderInfo.orderInfos">
														<td>
															<div style="overflow: hidden;" v-for="detail in orderInfoc.orderDetailInfos">
																<div style="width: 50px; height: 50px; float: left;">
																	<a href="产品url" target="_blank">
																		<img width="50" height="50" :src="detail.productImage">
																	</a>
																</div>
																<div style="margin-top: 7px;">
																	<a :title="detail.productName" href="产品url" target="_blank">
																		{{detail.productName}}
																	</a>
																</div>
																<div>
														...
														<td>
															<div>商品金额: ￥{{orderInfo.order_total_price}}</div>
															<div v-if="orderInfo.freight_price>0">运费金额:￥{{orderInfo.freight_price }}</div>
															<div>总金额: ￥{{orderInfo.order_total_price}}</div>
														</td>
														<td>
														...
														<td>
															...
															<span v-else-if="orderInfo.orderStatus==5">交易中途关闭</span>
															<span v-else-if="orderInfo.orderStatus==6">退货</span>
															<span v-else-if="orderInfo.orderStatus==7">锁定</span>
															<span v-else-if="orderInfo.orderStatus==11">系统生成退款单</span>
															<span v-else-if="orderInfo.orderStatus==12">财务退款操作</span>
															<span v-else-if="orderInfo.orderStatus==101">更新配送费用</span>
															<span v-else-if="orderInfo.orderStatus==102">更新折扣</span>
														</td>
														<td>
															<span v-if="orderInfoc.expressName==null">暂无信息</span>
															<span v-else>快递名:{{orderInfoc.expressName}}<br>快递编号:{{orderInfoc.expressNumber}}</span>
														</td>
														...
														<td>
															<a target="_blank" href="店铺url" title="进入店铺">
																<i class="fa fa-exchange"></i>
															</a>
															<a target="_blank" href="订单详情url" title="查看详情">
																<i class="fa fa-eye"></i>
															</a>
														</td>
													</tr>
			...
			methods : {
				...
				add : function(){
					layer.open({
						...
						shade : 0.2,
						area : [ "70%", "80%" ],
						content : "orderInfo/goOrderInfoEdit.html"
					});
				},
				edit : function(id){
					layer.open({
