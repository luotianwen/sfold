											...
											<table class="table table-striped table-hover">
												...
												<tbody>
													<tr v-for="(syspchomeadware,index) in syspchomeadwares">
														<td>{{syspchomeadware.id}}</td>
														<td>{{syspchomeadware.ad_id}}</td>
														<td>{{syspchomeadware.imgLink}}</td>
														<td>
															<span v-if="syspchomeadware.top_pic==''">无</span>
															<span v-else><img :src="syspchomeadware.top_pic"></span>
														</td>
														<td>{{syspchomeadware.sort}}</td>
														<td>{{syspchomeadware.sku}}</td>
														<td>
															<span v-if="syspchomeadware.state==1">启用</span>
															<span v-if="syspchomeadware.state==2">禁用</span>
														</td>
														<td>{{syspchomeadware.lasttime}}</td>
														<td>{{syspchomeadware.updatename}}</td>
														<td>{{syspchomeadware.remark}}</td>
														<td>
															<button type="button" @click="edit(syspchomeadware.id,syspchomeadware.ad_id)" class="btn btn-outline btn-primary">
	...
	<script type="text/javascript">
		new Vue({
			...
			methods : {
				...
				edit : function(id,ad_id){
					layer.open({
						type : 2,
						title : "编辑",
						shade : 0.2,
						area : [ "70%", "80%" ],
						content : "syspchomeadware/goSysPcHomeadWareEdit.html?id=" + id+"&&ad_id="+ad_id
					});
				},
				submit : function(){
